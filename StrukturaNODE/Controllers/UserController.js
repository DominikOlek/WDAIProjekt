...
const register = (async (req, res) => {
    try {
        ...
        if (isBe != null) {
            return res.status(406).send("An account with this email address already exists");
        }
...
const refresh = (async (req, res) => {
    try {
        ...
        if (token == null || check == null) {return res.sendStatus(401);}
...
function generateJWT(data) {
    ...
    return jwt.sign(claims, process.env.TOKEN_SECRET, { expiresIn: '1000s' }); //!!!DO TESTÓW
}

function generateRefresh(data) {
    const claims = { id: data.ID, username: data.Email, name: data.Name, lastname: data.LastName, role: data.Role };
    return jwt.sign(claims, process.env.TOKEN_REFRESH, { expiresIn: '90000s' }); //!!!DO TESTÓW
}
...
module.exports = { login, register, confirm, getAllUser ,setRole ,refresh,logout};